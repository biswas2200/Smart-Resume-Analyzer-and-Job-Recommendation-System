...
| **Python ML Service** | Embedding generation, skill normalization, cosine-similarity role matching, deterministic ATS scoring, gap analysis, and the two narrowly-scoped LLM calls (extraction, explanation). | **Implemented** (`ml-service/`) |
| **Groq LLM API** | External inference provider. Used only for resume-text extraction and natural-language explanation generation — never for scoring or ranking. | Implemented (integration point) |
...
| `services/parser.py` | Resume text → `ResumeProfile`. One LLM call, extraction-only system prompt, strips markdown code fences from the response before JSON-parsing it. | **Yes** |
...
| `services/llm_client.py` | `LLMClient` Protocol + `GroqLLMClient` implementation. Both `parser.py` and `explainer.py` depend on the Protocol, not the Groq SDK directly — this is what makes them unit-testable with a `FakeLLMClient` and no network access (see `tests/test_pipeline.py`). | — (abstraction) |
| `config.py` | `pydantic-settings`-based config: `GROQ_API_KEY`, `GROQ_MODEL`, `EMBEDDING_MODEL`, loaded from `.env`. | — |

## 3. Key design decisions

### 3.1 Deterministic core, narrow LLM footprint
Every score or ranking the user sees is computed by plain code. The LLM touches exactly two things: turning unstructured resume text into structured JSON, and turning structured results into prose. This directly implements SRS §2.5's constraint ("core matching and scoring logic must be deterministic and explainable") and is the system's answer to the comparable-systems gap cited in SRS §1.4 (static, unreproducible LLM-only matching).

### 3.2 LLM access behind a Protocol, not a concrete SDK
`LLMClient` is a `typing.Protocol` with one method, `complete(system_prompt, user_prompt) -> str`. `parser.py` and `explainer.py` take an optional `client` parameter that defaults to `get_llm_client()` (a cached `GroqLLMClient` singleton). This means: (a) the provider can be swapped without touching business logic, and (b) the full test suite runs with zero network access and no API key, using a `FakeLLMClient`.
...
NFR-3.1: if the Groq API is unavailable, the system returns deterministic match/ATS scores without a natural-language explanation rather than failing the whole request. This falls out naturally from the architecture — `/match` and `/ats-score` never call the LLM at all, and `/analyze` can be adapted to catch an `explainer` failure and return the rest of the payload with `explanation` empty.
...
| ML Service → Groq API | HTTPS / REST | JSON (chat completion) | API key (env var, never committed) |
